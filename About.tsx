...
export const About: React.FC = () => {
  const [experience, setExperience] = useState<AboutContent | null>(null);
    ...
    <Section className="pt-32">
      ...
      {loading ? (
        ...
        <div className="mt-32 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <Heading level={2} className="mb-6">{experience.experience_title}</Heading>
            <div className="prose prose-invert prose-lg text-slate-400 mb-8">
              {experience.experience_body.split('\n\n').map((paragraph, idx) => (
                <p key={idx} className="mb-6">{paragraph}</p>
              ))}
            </div>
            {experience.experience_bullets && experience.experience_bullets.length > 0 && (
              <div className="space-y-4">
                {experience.experience_bullets.map((bullet, idx) => (
                  <div key={idx} className="flex items-start text-white/90">
          ...
          <div>
            {experience.experience_image_url && (
              <GlassCard className="p-3 bg-white/5">
                <img
                  src={experience.experience_image_url}
                  alt={experience.experience_image_alt || "Experience"}
